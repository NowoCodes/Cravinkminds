          ...
          <div class="container">
            <div class="row ux">
              ...
              if (isset($_GET['search'])) {
                ...
                $pp="SELECT *  FROM news where MATCH(head,news) against('$query') ORDER BY `time` DESC";
                 $rpp=mysqli_query($conn,$pp);
                 $months=['January','February','March','April','May','June','July','August','September','October','November','December'];
                  if (mysqli_num_rows($rpp)==0) {
                    echo "<p>Your search does not match any news <br> <a href='news.php'>All News</a></p>";
                }
                else{
                  ...
                  echo "<div class='post-details'>
                        ...
                        <a href='new.php?id=$id'><h3 class='h4'>$head</h3></a>
                  ...
                  ";}
                    ...
                    echo " <div class='post-thumbnail'><a href='new.php?id=$id'><img src='img/news/$img' alt='...'' class='img-fluid'></a></div>
              ...
              if (isset($_GET['pg'])) {
                ...
                $pp="SELECT *  FROM news ORDER BY `time` DESC LIMIT $off,8 ";
                 $rpp=mysqli_query($conn,$pp);
                 $months=['January','February','March','April','May','June','July','August','September','October','November','December'];
                 while ($gpp=mysqli_fetch_array($rpp)) {
                  $id=$gpp['id'];
            ...
            <nav aria-label="Page navigation example">
              <ul class="pagination pagination-template d-flex justify-content-center">
                 ...
                 if (isset($_GET['search'])) {
                  ...
                  $no=mysqli_query($conn,"SELECT COUNT(id) as total FROM news where MATCH(head,news) against('$query')");
                  $nos=mysqli_fetch_assoc($no);
                  $num=ceil($nos['total']/8);
                  for ($i=1; $i<$num+1 ; $i++) { 
                    $i=$i;
                    echo "<li class='page-item'><a href='news.php?search=$query&pg=$i' class='page-link active'>$i</a></li>";
                  };
                  ...
                  $no=mysqli_query($conn,"SELECT COUNT(id) as total FROM news");
                  $nos=mysqli_fetch_assoc($no);
                  $num=ceil($nos['total']/8);
                  for ($i=1; $i<$num+1 ; $i++) { 
                    $i=$i;
                    echo "<li class='page-item'><a href='news.php?pg=$i' class='page-link active'>$i</a></li>";
                  };

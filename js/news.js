...
$('.left').click(function(e){
	...
	if (n==1){
		window.open('news.php?pg=1','_self');
	}
	else{
		nn=parseInt(n)-1;
		window.open('news.php?pg='+nn,'_self');
	};
	...
	if (pos==0){
			...
			if (no==1) {
				window.open(url,'_self');
					...
					window.open('news.php?pg=2','_self');
				}	
		}
	else{
		if (n==no){
		window.open('news.php?pg='+no,'_self');
	}
...
$(document).ready(function(){
	$('.commbut').click(function(e){
		...
		if(comment!=''){
			$.post('ncomment.php',{id:id,cravinkuname:uname,comment:comment},function(data,status){
			if (data=='Success'){
				$('.ment').val(' ');
